...
data = {
    ...
    'team1': parser.find_all('div', class_='mr-2')[0].text,
    'score1': parser.find_all('div', class_='mr-2')[0].find_next_sibling('div').text,
    'team2': parser.find_all('div', class_='mr-2')[1].text,
    'score2': parser.find_all('div', class_='mr-2')[1].find_next_sibling('div').text,
    'result': teams[parser.find_all('div', class_='text-cbTextLink')[0].text.split(' won')[0]] + ' won' + parser.find_all('div', class_='text-cbTextLink')[0].text.split(' won')[1].replace('wkts', 'wickets'),
    'stadium': parser.find_all('a', class_='hover:underline')[1].text.split(', ')[1],
    'player_of_match': parser.find('div', class_='rounded-full overflow-hidden border border-solid border-gray-400 w-avatar').find_next_sibling().text
}

scorecard = 'https://www.cricbuzz.com/live-cricket-scorecard/' + match_id
...
with open('scraper/names.json', 'r') as file:
...
response = requests.post("http://localhost:3000/api/admin/update", json=data)
